...
1.  Exploring **quanteda** functions.
    ...
    You can try running `demo(quanteda)`, and also use the `example()` function for any 
...
1.  Making a corpus and corpus structure
    ...
    1.  From a directory of text files.
        ...
        The `corpus()` function can take as its main argument the name of a directory,
        if you wrap the path to the directory within a `directory()` call.  (See `?directory`
        for an example.)  If you call `directory()` with no arguments, then it should allow
        you to choose the directory interactively (you will need to have installed the
        `tcltk2` package first though.)
        ...
        Note that if you document level metadata in your filenames, then this can be 
        automatically parsed by `corpus.directory()` into `docvars`.
...
1.  **Preparing and pre-processing texts**
    ...
    1. "Cleaning"" texts
        ...
        documentation for **quanteda**}'s `clean` command `?clean` and use the
    ...
    1.  Tokenizing texts
        ...
        documentation for **quanteda**'s `tokenize` command using the
        built in help function (`?` before any object/command).  Use the 
...
1.  **Descriptive statistics**
    ...
    5.  `summary` `quanteda` provides a function to count syllables in a
...
3.  **Lexical Diversity over Time**
    ...
    3.  Now compare the results from the `lexdiv` function applied to the 
        texts.  Are the results the same?

4.  **Document and word associations**

    1.  Load the presidential inauguration corpus selecting from 1900-1050,
        and create a dfm from this corpus.
    
    2.  Measure the document similarities using `similarity()`.  Compare the results
        for Euclidean, Euclidean on the term frequency standardized dfm,
        cosine, and Jaccard.
        
    3.  Measure the term similarities for the following words: *economy*, *health*,
        *women*.
...
1.  **Working with dictionaries**
    ...
    1.  **Creating a simple dictionary.**
        ...
        Weight the `posDfm` by term frequency using `tf()`, and plot the values of articles
    ...
    1.  **Replicating a published dictionary analysis**
        ...
        document.  Hint: Use `weight(x, "relFreq")` on the dfm.
